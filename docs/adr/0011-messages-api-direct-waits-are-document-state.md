...
6. **#17이 정하지 않은 전이는 이름 붙은 거부로 막는다.** ~~반려·보류 이후, 재심 복귀, 타임아웃이 그 자리다.~~ **정정(#10, 2026-09-22)**: ~~반려 이후(`CONTEXT.md:81`), 답 없이 시간이 흐른 보류(`CONTEXT.md:88`), 재심 복귀, 타임아웃이 그 자리다.~~ ~~보류 → 답변 → 같은 대기 복귀는 정의된 전이라 막지 않는다~~(`docs/adr/0003-approver-actions-three.md:25-26`, `docs/research/agent-architecture-comparison.md:158`). **정정(#17, 2026-09-22)**: 보류 답변이 같은 대기로 돌아가는 것은 입력을 바꾸지 않았거나 결재선과 앞선 승인 카드가 그대로일 때다(`docs/adr/0003-approver-actions-three.md:26`). 입력을 바꾼 답변은 [ADR-0026](0026-hold-answer-deadline-time-changes-no-state.md) 결정 7을 따른다. #17이 그 넷을 정했다([ADR-0024](0024-rejection-resubmission-and-card-bound-approvals.md)·[ADR-0026](0026-hold-answer-deadline-time-changes-no-state.md)·[ADR-0027](0027-reopen-returns-to-first-finance-consent.md)). ~~이 거부는 #17이 다른 티켓에 넘긴 세 자리에만 남는다 — 재무합의자 0명 정산의 재심, 재심 정정 뒤 차액의 지급 처리(#20), 재량 판단이 결재선 입력을 바꾼 뒤의 승인·동의(#21).~~ **정정(#21, 2026-09-23)**: 이 거부는 #17이 #20에 넘긴 두 자리에만 남는다 — 재무합의자 0명 정산의 재심, 재심 정정 뒤 차액의 지급 처리. 재량 판단의 자리는 [ADR-0029](0029-approval-line-from-claimed-values.md)가 닫았다. #17이 이유를 정한 거절에는 이유를 말하는 이름을 붙인다([ADR-0023](0023-document-and-evidence-axes-named-rejections.md) 결정 8).
