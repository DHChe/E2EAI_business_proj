...
- 상신 뒤 새로 생긴 짝 보류(A10)·`명세 대기`와 재무합의자에게 배정된 짝 확인(값 모순)도 현재 단계의 승인·동의만 막는다(C3, [ADR-0031](adr/0031-reconciliation-pending-states-and-input-driven-rerun.md) 결정 4). 문서 상태는 바꾸지 않는다. 이 대기를 도메인 용어로 올릴지는 #22다.
...
`명세 대기`는 이용 내역이 그 날짜를 아직 덮지 않은 중간 상태이고 사람에게 가지 않는다(날짜 후보가 여럿이면 수기 확정이 먼저다). 상신 뒤에는 짝 보류와 같은 가드를 따른다. 대사는 입력(대사 입력 필드·카드 줄 revision, 이용 내역의 수집 기준일)이 바뀔 때만 다시 돌고, 이의 플래그만으로는 돌지 않는다. 사람의 짝 확인은 그 경쟁 묶음(좁히기 전 후보로 이어진 증빙·카드 줄)의 입력이 결정 뒤 바뀌지 않은 동안 효력이 있고, 바뀌면 묶음의 결정이 함께 효력을 잃는다. 수집 기준일의 전진은 결정을 무르지 않는다. 확인 사슬은 묶음마다 하나씩 진행하고, 짝 확인의 답은 연 때의 묶음 판을 기대 revision으로 실어 어긋나면 `STALE_REVISION`이다. 같은 묶음에 완료되지 않은 문서의 증빙이 있으면 완료 문서의 증빙도 짝 확인에 들지만 그 문서는 움직이지 않는다. 재심의 "바뀐 카드"는 원 승인이 읽은 카드와 지금 카드의 차이이고 재심 전에 대사가 바꾼 것도 든다 — 그런 건의 재심은 아래 재심 행에서 바뀐 카드가 있는 쪽을 따른다([ADR-0031](adr/0031-reconciliation-pending-states-and-input-driven-rerun.md) 결정 3–5).
...
| `재무합의대기`(k) | 동의 | 재무합의자 k | 현재 단계 담당자, 자기 결재 아님. 재량 관문·상신 뒤 짝 보류·`명세 대기`·배정된 짝 확인(값 모순)이 없다. 재량 판단이 결재선 입력을 바꾸지 않았다(바꿨으면 아래 #21 거절 행). 정산 재심 표지가 없다(있으면 아래 재심 행) | `재무합의대기`(k+1), 마지막이면 `결재대기`(1) | 권한·상태 거부(§5) | `CONTEXT.md:66`, `docs/adr/0012-transition-audit-provenance-in-one-transaction.md:35`, `:46` |
...
| 정산 재심 표지가 붙은 대기 | 재심 단계의 동의·승인 | 그 단계 승인자 | 현재 단계 담당자, 자기 결재 아님. 재량 관문·상신 뒤 짝 보류·`명세 대기`·배정된 짝 확인(값 모순)이 없다. 재량 판단이 결재선 입력을 바꾸지 않았다(바꿨으면 위 #21 거절 행). 재심 표지는 이 행의 출발 조건이다 | 재심 범위에 다음 단계가 남았으면 그 단계. 범위의 마지막 단계에서는 원 승인이 읽은 카드와 달라진 카드가 없으면(재심 전 대사 변동도 든다, ADR-0031 결정 5) 복귀점과 원 완료 시각(지급 뒤 상태로의 복귀는 `[설계 가정]`), 바뀐 카드가 있으면 새 `완료`와 새 완료 시각. 재무 범위만의 쟁점이면 범위의 마지막은 마지막 재무합의자다. 카드가 바뀌면 범위는 결재선 끝까지 넓어진다 | — | ADR-0027 결정 2·4·7. #17 표 3행 |
...
**§7.5 요구 칸 대조.** `docs/design/receipt-pipeline.md` §7.5(`:2383-2391`)의 일곱 칸은 모두 충족하고, `CardLine`(`:1817-1830`)이 요구하는 둘을 더 준다(ADR-0021 결정 8). `카드 사용자`와 수집 기준일은 #26이 더했다([ADR-0031](adr/0031-reconciliation-pending-states-and-input-driven-rerun.md) 결정 4·7).
...
청구 명세의 줄은 이용 내역의 줄에 잇고, 대사 입력 칸이 다르면 카드 줄의 새 revision으로 받아 대사가 다시 돈다(`:2394-2397`) `[설계 가정]`.
