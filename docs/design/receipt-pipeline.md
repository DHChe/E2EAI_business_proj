...
- **사람이 고른다**: `후보 복수`와 `짝 보류`는 자동으로 묶지 않은 중간 상태다. 누가 확인하는지는 막힌 이유가 정한다 —
  ...
  **경쟁 확인은 "이 카드 줄은 어느 증빙인가"를 정한다**(#26). 그 줄을 노린 증빙(`claims`) 모두에 기록을 남긴다 — 고른 증빙은 `같은 지출`, 나머지는 `이 줄 아님`이고, `후보 복수`인 증빙도 든다.
  진 증빙은 그 줄만 잃고 남은 후보로 다시 대사한다(`withoutDecidedLines`). 효력 있는 `같은 지출`로 짝이 선 줄은 다른 증빙의 후보에서 빠지고, 짝이 선 증빙은 다른 줄의 `claims`에서 빠진다 — 한 카드 줄에 짝이 둘 서지 않고, 이미 선 증빙이 다른 줄의 경쟁을 만들지 않는다.
  결정이 효력이 있는지는 좁히기 전의 입력으로 본다 — `candidateLines`가 낸 후보와 그것으로 모은 `claims`다. 좁힌 값으로 보면 결정이 선 순간 입력이 달라져 스스로 효력을 잃는다.
 ...
 * 효력 있는 `같은 지출`로 다른 증빙에 선 줄을 뺀다. 그렇게 선 줄의 `claims`는 그 증빙 하나이고, 짝이 선 증빙은 다른 줄의 `claims`에서 빠진다.
 */
