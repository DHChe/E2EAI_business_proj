...
| 지표의 화면·주기·임계 운영 | #11 |
| 수기 확정 후 재개, 값 수정 후의 상태 전이, admin 재심이 대사를 다시 거치는 조건 | #17 — 값 수정은 새 필드 revision으로 검산·대사를 다시 돌리고([ADR-0023](../adr/0023-document-and-evidence-axes-named-rejections.md) 결정 2), 문서는 카드가 바뀐 가장 앞 단계로 되돌린다([ADR-0024](../adr/0024-rejection-resubmission-and-card-bound-approvals.md) 결정 5). 재심은 값이 바뀔 때만 대사를 다시 거치고 플래그만으로는 다시 돌리지 않는다([ADR-0027](../adr/0027-reopen-returns-to-first-finance-consent.md) 결정 7–8) |
