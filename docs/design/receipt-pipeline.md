...
| 순위 | 키 | 성질 |
| --- | --- | --- |
| 1 | **승인번호** (양쪽에 있을 때) | 거래 식별자. 하나면 후보 탐색은 거기서 끝난다 — 자동 확정은 아래 세 조건을 따로 거친다 |
...
- **금액은 후보 집합으로 비교한다.** 구분자 해석이 둘이거나(§4.3) 手記 팁 후보가 있으면 그 전부를 후보로 넣고,
  **정확히 하나가 카드 줄과 맞으면 그 순간 추출 불확실이 닫힌다.** 사람에게 가지 않는다.
  ...
  이 자기 확인의 위험은 조건 1(양방향 경쟁 후보 0)과 끝4·날짜 창 일치가 막는다.
...
| 2. 해결되지 않은 추출 문제 0 | 짝의 근거인 카드 끝4·금액·날짜가 대조로 닫혔다. 대사가 카드 줄 값으로 후보 집합을 정확히 하나로 좁혔으면 해결로 친다. 막는 것은 **대조로 닫히지 않는 추출 문제뿐이다** — 예: 금액·날짜를 판독하지 못해 대조 없이 승인번호로만 찾은 후보 | `짝 보류` |
| 3. 값끼리의 모순 0 | 증빙 값과 카드 줄 값이 충돌하지 않는다 — 금액 · 날짜 창 · 가맹점 사업자등록번호 · 가맹점 소재국 | 금액만 충돌하면 기존대로 `대사 불일치`, 그 밖에는 `짝 보류` |

- **`짝 보류`** 는 후보가 하나지만 자동 확정 조건에 막혀 사람이 확인하는 상태다. 막힌 조건을 함께 남긴다.
  `후보 복수`는 그대로 후보가 둘 이상 남은 상태다.
...
/** 짝의 근거가 되는 필드. 자동 확정 조건 2가 이 셋이 대조로 닫혔는지 본다. */
export type PairField = "카드 끝4" | "금액" | "날짜";

/** 자동 확정을 막은 조건. `짝 보류`가 이것을 함께 남긴다. */
 ...
 * 1 경쟁 후보 0(양방향) · 2 대조로 닫히지 않은 추출 문제 0 · 3 증빙 값과 카드 줄 값의 모순 0.
 * 어느 층에서 찾은 후보든 같은 조건을 거친다.
 *
...
export function decideMatch(
  ...
  if (amountDifference !== null) {
    // 금액만 충돌한다 — 짝은 서고, 차액은 확정된 사실로 남는다(§7.4).
...
| **금액 후보 확대**(구분자 2개 해석·팁 후보)가 엉뚱한 줄과 맞는다 | exponent 3 통화, 통화 미상 | 후보가 둘 이상 맞으면 자동 확정하지 않는다. 하나만 맞으면 그 카드 줄이 값을 풀고 짝도 확정하는 자기 확인이 되므로, 조건 1(양방향 경쟁 후보 0)과 끝4·날짜 창 일치가 막는다(§7.2) |
...
| **승인번호 우선**이 잘못된 짝을 확정한다 | 카드사가 승인번호를 재사용하는 경우 | 승인번호가 붙어도 금액을 비교해 다르면 `대사 불일치`로 남긴다. 날짜 창·가맹점 사업자등록번호·소재국이 어긋나면 조건 3이, 금액·날짜를 판독하지 못해 대조 없이 승인번호로만 찾았으면 조건 2가 `짝 보류`로 막는다 |
...
- **사람이 고른다**: `후보 복수`와 `짝 보류`는 자동으로 묶지 않은 중간 상태다.
  사람이 원본과 카드 줄을 한 화면에서 보고 같은 지출이라고 확인하면 짝이 선다 — 금액이 맞으면 `대사 완료`, 어긋나면 `대사 불일치`이고
  짝의 근거는 `사람 선택`이다. 같은 지출이 아니라고 하면 `짝 없음`이 된다.
  `짝 보류`는 막힌 조건(`blockedBy`)을 그 화면에 함께 보인다.
...
| --- | --- | --- | --- | --- | --- |
| 1 | 베트남 식당, 통화기호 없이 `TỔNG CỘNG 1.250.000`, 법인카드 | `total.raw="1.250.000"`, `value="1250000"`, `status=read` / `currency.raw=""`, `value=null`, `status=absent` | 코드 파싱 = 후보 1개(`repeated_separator`, 실행) → 모델 값과 일치. G1: 항목합 = 합계 → **검산 성공** | **없음.** 통화 후보는 대사에서 닫힌다(카드 줄의 원거래 통화 VND) | `합계 1,250,000 VND · 통화: 시스템 연동(법인카드 명세) · 금액: 교차 확인` / `수취 의무: 의무 면제 — 국외 공급(법인세법 시행규칙 §79 4호)` |
| 2 | 독일 호텔 인보이스 `1.234,56 EUR`, 법인카드 | `total.raw="1.234,56 EUR"`, `value="1234.56"`, `status=read` | 코드 파싱 = `1234.56` 후보 1개(`two_separators`, 실행). 독일 고유 검산 규칙 없음 → KR-1·JP-1 **불가**, G1: 항목합 = 합계 → **성공** | **없음** | `합계 1,234.56 EUR · 에이전트 추출 · 교차 확인` / `한국 부가가치세: 해당 없음 — 국외 공급이라 과세대상이 아니다(부가가치세법 §4·§20①1호·§38①1호)` |
| 3 | 멕시코 식당, 항목 150.00 + 150.00, `IVA 48.00`, `TOTAL 348.00`, 포함/별도 표기 없음, 법인카드 | `tax_lines[0].inclusion="unclear"`, `total.value="348.00"` | G1이 가설 두 개를 대조 → `항목합 + 세액 = 합계`만 맞음(실행) → **검산 성공**, 별도 표기로 확정 | **없음.** 해외 간접세는 판정 입력이 아니므로 포함/별도가 끝내 불분명해도 사람에게 보내지 않는다 | `합계 348.00 MXN · 교차 확인 · 검산 확인(항목합 + 세액 = 합계)` / `현지 간접세 IVA 48.00 MXN — 한국 매입세액과 다른 칸에 기록` |
| 4 | 파리 경유 식사, 항목 39,00 + 9,50, `dont TVA 10% 4,41`, `Service compris`, `TOTAL TTC 48,50 €`, 법인카드 | `total.value="48.50"`(`digit_count`, 실행), `service_included_notice=yes`, `tip.status=absent` | FR-1이 `소계 + 세액 + 팁` 가설을 끈다 → G1: 항목합 = 합계 → **검산 성공**. TVA는 포함이므로 더하지 않는다 | **없음** | `합계 48.50 EUR · 교차 확인 · 검산 확인(항목합 = 합계, service compris)` / `수취 의무: 의무 면제 — 국외 공급(시행규칙 §79 4호)` |
...
| 6 | 도쿄 편의점 `合計 ¥1,630`, `8%対象 ¥1,080(内消費税 ¥80)`, `10%対象 ¥550(内消費税 ¥50)`, 법인카드 | `total.raw="¥1,630"` → exponent 0이라 후보 1개 `1630`(`minor_unit`, 실행). `tax_lines` 2행, `inclusion=included` | JP-1: 8% → 1080×8/108 = 80.0, 10% → 550×10/110 = 50.0, 과세표준 합 1,630 = 합계 → **검산 성공**(실행) | **없음** | `합계 1,630 JPY · 교차 확인 · 검산 확인(세율별 구분 기재)` / `현지 소비세 8%·10% 두 줄로 보존` |
| 7 | 멕시코 식당 카드전표, 인쇄 `TOTAL 520.00`, 손글씨 `Propina 80.00` / `TOTAL 600.00` | `total_candidates` 2개(printed 520.00, handwritten 600.00), `tip.value="80.00"`, `tip_origin=handwritten` | TIP-1: 520 + 80 = 600 → **손글씨 최종액 선택**(실행). 인쇄값을 잡았다면 대사가 전부 틀어졌을 자리다 | **없음** | `합계 600.00 MXN(손글씨 최종액, 팁 80.00 포함) · 교차 확인` |
...
| H6 | 합계를 못 읽었고 짝도 없는 건 | 필드 확정이 아니라 **재촬영 요청** | 하드 규칙 | 근거 없는 수기 입력을 막는다 |
| H7 | 국외 공급 건의 법정 판정 | **판정 불가 0건** | 법령의 귀결 | 시행규칙 §79 4호에서 판정이 끝난다 |
| H8 | 법인카드 결제 + 대사 완료 건의 법정 판정 | **판정 불가 0건, 수취 의무 미충족 0건** | 법령의 귀결 | 시행령 §158④ |
...
/**
 * 사람에게 보낼지 정한다. 규칙은 셋뿐이다 —
 * ① 합계를 읽지 못했고 대사도 붙지 않으면 필드 확정이 아니라 재촬영이다.
...
export function routeToHuman(
  evidenceId: string,
  states: FieldState[],
  options: { totalUnreadable: boolean; reconciled: boolean; now: string },
): HumanRouting {
  if (options.totalUnreadable && !options.reconciled) {
    return {
      kind: "재촬영 요청",
      reason: "합계를 읽지 못했고 카드 명세에도 짝이 없다 — 필드를 하나씩 받아도 근거가 남지 않는다",
    };
  }
...
| # | 지표 | 분자 / 분모 | 상한 | 성질 |
| --- | --- | --- | --- | --- |
| R1 | 수기 필요율 | 수기 확정 대상으로 판정된 고유 지출(대기 포함) / 접수된 고유 지출 | **≤ 10%** | `[초기 검증 가정]` |
| R2 | 추가 주의 요구율 | 수기 확정 대상이거나 승인 카드의 주의 신호(H11 — (ii) 법적 하자 · 판정 불가)로 별도 판단을 요구한 고유 지출의 합집합 / 접수된 고유 지출 | **≤ 15%** | `[초기 검증 가정]` |
| R3 | 사람 접촉 횟수 | 수기 요청 + 재수정 요청 + 추가 확인 + 감사 표본 요청의 실제 횟수 / 접수된 고유 지출 | **100건당 ≤ 20회** | `[초기 검증 가정]` |
| R4 | 책임자 하루 추가 작업 | 책임자별로 실제 배정된 추가 수기·확인·감사 작업 수 | **1인 하루 ≤ 20회** | `[초기 검증 가정]` |
